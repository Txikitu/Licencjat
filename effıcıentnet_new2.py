...
MODEL_PATH = "efficientnetnew.model"
...
validgen = train_generator.flow_from_directory(DATA_DIR/'train', target_size=(600, 600),
                                               class_mode='categorical',
                                               classes=classes,
                                               subset='validation',
                                               batch_size=BATCH_SIZE,
...
base_model = efficientnet.EfficientNetB7(weights='imagenet', include_top=False, input_shape=(600, 600, 3))
...
H = model.fit(traingen, steps_per_epoch=traingen.samples // BATCH_SIZE, validation_data=validgen,
              validation_steps=validgen.samples // BATCH_SIZE, epochs=NUM_EPOCHS)
...
resnet_acc = accuracy_score(true_classes, predIdxs)
print("VGG16 Model Ac curacy with Fine-Tuning: {:.2f}%".format(resnet_acc * 100))

print("[INFO] saving model...")
model.save(MODEL_PATH, save_format="efficientneth5")
